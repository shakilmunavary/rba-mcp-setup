...
#!/bin/bash
# =============================================================================
# Minikube Monitoring Stack — NON-DOCKERIZED v1.1
# Grafana + Loki + Promtail + Alert Processor as native Linux systemd services
#
# FIXES vs v1.0:
#   - Flask installed with --ignore-installed blinker (Ubuntu 24 conflict fix)
#   - Routing policy uses X-Disable-Provenance: true (Grafana 13 requirement)
#   - Processor wait time increased to 60s
#   - Routing policy fetches actual default receiver name before applying
# =============================================================================

# =============================================================================
# SECTION A — APP PROFILES
# =============================================================================
...
DEVOPS_AGENT_CONSOLE_URL="https://us-west-2.console.aws.amazon.com/aidevops/home?region=us-west-2#/agent-spaces/37e6e8a9-2900-46b8-a550-f20d0412421b"
...
INSTALL_DIR="/opt/devops-monitoring"
LOKI_VERSION="3.4.2"
...
echo ""
echo "══════════════════════════════════════════════════════"
echo "  Monitoring Stack — Non-Dockerized v1.1"
echo "══════════════════════════════════════════════════════"
...
# FIX: install flask with --ignore-installed blinker to avoid Ubuntu 24 conflict
...
python3 -c "import flask, boto3" \
    ...
    || { echo "[ERROR] flask/boto3 still not importable"; exit 1; }

mkdir -p $INSTALL_DIR/{loki,promtail,grafana,processor,data}
...
root_url = https://devopsatkmcptools.com/
...
# Build namespace-specific scrape jobs
SCRAPE_CONFIGS=""
...
# Fallback: scrape all pods with namespace label extracted from path
SCRAPE_CONFIGS="${SCRAPE_CONFIGS}
...
# =============================================================================
step "PHASE 6 — Alert Processor (Flask systemd service)"

# Build app profiles JSON
APP_PROFILES_JSON="["
FIRST=true
for i in "${ACTIVE_APPS[@]}"; do
...
DEVOPS_AGENT_CONSOLE = os.environ.get('DEVOPS_AGENT_CONSOLE_URL',  '')
DEFAULT_SRE_EMAIL    = os.environ.get('SRE_EMAIL',            '')
SES_SENDER           = os.environ.get('SES_SENDER_EMAIL',     '')
PROFILES_FILE        = os.environ.get('PROFILES_FILE', '/opt/devops-monitoring/processor/profiles.json')
...
def send_email(inc_number, title, ns, pods, logs, inv_id, jdet, profile, rca_content=''):
    ...
    agent_link = DEVOPS_AGENT_CONSOLE
    if inv_id and inv_id != inc_number:
        agent_link += '?investigationId=' + urllib.parse.quote(str(inv_id))
    ...
    rca_html = (
        ...
        'margin:16px 0"><p style="margin:0">⏳ Agent investigation in progress. '
        'Click <b>View Agent Investigation</b> for live RCA status.</p></div>'
    )
...
<h2 style="margin:0">🚨 Production Incident — {profile.get("alert_name",ns)}</h2>
...
<tr><td style="padding:8px;background:#f9f9f9;font-weight:bold;width:140px;border:1px solid #ddd">Incident</td>
    <td style="padding:8px;border:1px solid #ddd"><a href="{snow_link}" style="color:#1565c0">{inc_number}</a></td></tr>
...
<tr><td style="padding:8px;background:#f9f9f9;font-weight:bold;border:1px solid #ddd">Jenkins Build</td>
    <td style="padding:8px;border:1px solid #ddd;color:{jcolor}">#{jdet.get("build_number","N/A")} — {jdet.get("build_status","N/A")}</td></tr>
...
<a href="{snow_link}" style="background:#1565c0;color:white;padding:12px 20px;text-decoration:none;border-radius:6px;margin:4px;display:inline-block">📋 ServiceNow</a>
<a href="{agent_link}" style="background:#2e7d32;color:white;padding:12px 20px;text-decoration:none;border-radius:6px;margin:4px;display:inline-block">🤖 Agent Investigation</a>
<a href="{jdet.get("console_url","#")}" style="background:#e65100;color:white;padding:12px 20px;text-decoration:none;border-radius:6px;margin:4px;display:inline-block">🔧 Jenkins Console</a>
<a href="{profile.get("github_repo","#")}" style="background:#424242;color:white;padding:12px 20px;text-decoration:none;border-radius:6px;margin:4px;display:inline-block">📦 GitHub</a>
</div>
<p style="color:#999;font-size:11px;text-align:center">DevOps Monitoring v1.1 — {CLUSTER_NAME} — {AWS_REGION}</p>
</div></body></html>'''
    ...
    try:
        ...
        for alert in alerts:
            ...
            threading.Thread(
                ...
                daemon=True).start()
            results.append({'action':'created','incident':inc_num,'agent_triggered':agent_ok})
...
def health():
    ...
    return jsonify({'status':'healthy','version':'non-docker-1.1','cluster':CLUSTER_NAME,
                    'monitored_apps':[{'name':p.get('alert_name'),'namespace':p.get('namespace')} for p in profiles],
                    'agent_webhook':bool(DEVOPS_AGENT_WEBHOOK),'boto3':b3})
...
def test_snow():
    ts = datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')
    num, sid = _create_ticket(f'[TEST] Non-Docker Monitor v1.1 — {ts}',
...
def test_email():
    prof = (load_profiles() or [{}])[0]
    ok, res = send_email('INC-TEST','[TEST] Email Pipeline v1.1',
...
if __name__ == '__main__':
    profiles = load_profiles()
    print(f'Alert Processor non-docker v1.1')
    print(f'SNOW: {SNOW_INSTANCE} | Agent: {bool(DEVOPS_AGENT_WEBHOOK)} | Email: {bool(DEFAULT_SRE_EMAIL and SES_SENDER)}')
...
Environment="DEVOPS_AGENT_CONSOLE_URL=${DEVOPS_AGENT_CONSOLE_URL}"
Environment="DEVOPS_AGENT_SPACE_ID=${DEVOPS_AGENT_SPACE_ID}"
...
Restart=always
RestartSec=5
StandardOutput=append:/var/log/devops-processor.log
StandardError=append:/var/log/devops-processor.log
...
# FIX: wait longer for processor (was 60s, increased to 90s)
...
# =============================================================================
step "PHASE 7 — Configure Grafana (datasource + alert rules + contact point + routing)"
...
# FIX: fetch actual default receiver name then apply routing with X-Disable-Provenance
...
PR_H=$(curl -s -o /dev/null -w "%{http_code}" --connect-timeout 3 \
    "http://localhost:${PROCESSOR_PORT}/health" 2>/dev/null || echo "000")
[ "$PR_H" = "200" ] && ok "Alert Processor: http://localhost:${PROCESSOR_PORT}" || warn "Processor: $PR_H"
...
echo ""
echo "══════════════════════════════════════════════════════"
echo "  ✅  NON-DOCKERIZED MONITORING STACK v1.1 READY"
echo "══════════════════════════════════════════════════════"
echo ""
echo "  📊 GRAFANA"
echo "     Local    : http://localhost:${GRAFANA_PORT}"
echo "     EC2      : http://${EC2_IP}:${GRAFANA_PORT}"
echo "     Domain   : https://devopsatkmcptools.com  (after ALB TG → port $GRAFANA_PORT)"
...
echo ""
echo "  🔧 ALB TARGET GROUP (manual — AWS Console)"
echo "     Type     : Instance   Protocol: HTTP   Port: $GRAFANA_PORT"
echo "     Health   : /api/health"
echo ""
echo "  🔍 ALERT FLOW"
echo "     /var/log/pods/ → Promtail → Loki (localhost:$LOKI_PORT)"
echo "       └─▶ Grafana checks every 1m (fires after ${ALERT_FOR_SECONDS}s)"
echo "             └─▶ Webhook → localhost:$PROCESSOR_PORT"
echo "                   ├─▶ SNOW ticket (deduped)"
echo "                   ├─▶ AWS DevOps Agent (HMAC webhook)"
echo "                   └─▶ RCA email after 90s"
echo ""
echo "  🛠️  SERVICE COMMANDS"
echo "     systemctl status grafana-server loki promtail devops-alert-processor"
...
echo "     curl -s 'http://localhost:${PROCESSOR_PORT}/test-loki?ns=${APP_1_NAMESPACE}&mins=60' | jq ."
echo ""
echo "     # Simulate critical error → triggers full pipeline"
echo "     kubectl exec -n ${APP_1_NAMESPACE} deploy/minikube-app -- \\"
echo "       /bin/sh -c 'echo \"CRITICAL: NullPointerException at app.Main\" >&2'"
echo ""
echo "     # Manual webhook fire"
echo "     curl -s -X POST http://localhost:${PROCESSOR_PORT}/webhook \\"
echo "       -H 'Content-Type: application/json' \\"
echo "       -d '{\"alerts\":[{\"status\":\"firing\",\"labels\":{\"severity\":\"critical\",\"namespace\":\"${APP_1_NAMESPACE}\"}}]}'"
echo ""
echo "  Log: $LOG_FILE"
echo "══════════════════════════════════════════════════════"
